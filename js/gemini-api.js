class GeminiAPI {
    constructor() {
        ...
        // 1. Environment variable (for Node.js or build-time injection)
        // 2. Window config object
        // 3. Fallback key
        this.apiKey = this._getApiKey();
        
        this.apiUrl = 'https://generativelanguage.googleapis.com/v1beta/models/gemini-2.0-flash:generateContent';
        this.isApiKeySet = !!this.apiKey;
        this.contextHistory = [];
    }
    ...
    _getApiKey() {
        ...
        // For browser with build-time environment variable injection
        // (This would be replaced by bundlers like Webpack/Vite)
        if (typeof GEMINI_API_KEY !== 'undefined') {
            return GEMINI_API_KEY;
        }

        // For browser with config object
        if (window.config?.geminiApiKey) {
            return window.config.geminiApiKey;
        }

        // Fallback key (remove this in production)
        return 'AIzaSyBkuotFw73RL2NLrdrsIGndlpjtlh-RruY';
    }
    ...
    loadApiKey() {
        ...
        const envKey = this._getApiKey();
        if (envKey && envKey !== 'AIzaSyBkuotFw73RL2NLrdrsIGndlpjtlh-RruY') {
            this.apiKey = envKey;
            this.isApiKeySet = true;
            return true;
        }
        
        // Fall back to localStorage if available and no env key
        if (typeof localStorage !== 'undefined') {
            ...
            try {
                ...
                if (jsonMatch) {
                    ...
                    if (!analysis.strengths) {
                        analysis.strengths = ['Clear communication detected'];
                    }
                    if (!analysis.improvements) {
                        analysis.improvements = ['Continue practicing for more improvements'];
                    }
